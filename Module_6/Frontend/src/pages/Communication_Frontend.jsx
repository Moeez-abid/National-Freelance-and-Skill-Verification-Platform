...
// ─── MESSAGES PAGE ────────────────────────────────────────────
  ...
  function MessagesPage({ userId, navigate, T, unreadAlerts, setUnreadAlerts }) {
    ...
    // ── Derived values ──
      ...
      // ── Handle chatWith URL parameter ──
      const processedChatWith = useRef(false);
      useEffect(() => {
        if (!userId || !contacts.length || processedChatWith.current) return;
        const params = new URLSearchParams(window.location.search);
        const chatWithId = params.get("chatWith");
        if (chatWithId && !isNaN(Number(chatWithId))) {
          processedChatWith.current = true;
          const targetId = Number(chatWithId);
          window.history.replaceState({}, "", window.location.pathname + "?userId=" + userId);
          
          const existing = rooms.find(r => r.room_type === "direct" && r.other_user_id === targetId);
          if (existing) {
            openDirectRoom(existing);
          } else {
            const c = contacts.find(x => x.id === targetId);
            const fakeRoom = { 
              room_id: null, 
              room_type: "direct", 
              room_name: c ? c.display_name : "User " + targetId, 
              other_user_id: targetId, 
              other_user_role: c ? c.role : "user", 
              online: c ? c.availability_status==="available" : false, 
              unread_count: 0, 
              members: [] 
            };
            setSelContact(fakeRoom); 
            setSelGroup(null); 
            setView("chat");
          }
        } else {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (urlToken) {
      ...
      window.history.replaceState({}, document.title, window.location.pathname + (id ? "?userId=" + id : ""));
    }
    
    if (!id || isNaN(Number(id))) {
      const token = urlToken || localStorage.getItem("token") || localStorage.getItem("m3_token") || sessionStorage.getItem("m3_auth_token");
      if (token) {
        try {
          const payload = JSON.parse(atob(token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')));
          id = payload.userId || payload.id;
        } catch (e) {
          console.error("Failed to decode JWT token for authentication:", e);
        }
      }
    }
