...
function TopNavBar({ onNavigate, role, currentUser, unreadCount, screen, theme, toggleTheme }) {
  ...
  const navItems = [
    ...
    { key: "search",     label: "Search",      external: "http://localhost:5004" },
  ];
  ...
  return (
    <aside className="fixed left-0 top-16 h-[calc(100vh-64px)] flex flex-col p-4 bg-slate-50 dark:bg-[#001736] w-64 z-40 border-r border-slate-200 dark:border-white/5 transition-colors duration-300">
      ...
      <div className="mt-auto space-y-4">
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-white/5 space-y-1">
          <button
            onClick={() => onNavigate("switch_role")}
            className="w-full flex items-center gap-3 px-3 py-2 text-slate-500 dark:text-slate-400 hover:text-primary transition-colors rounded-xl hover:bg-slate-100 dark:hover:bg-white/5"
          >
...
export default function App() {
    ...
    async function initApi() {
      ...
      if (found) {
        ...
        try {
          ...
          // Restore previously chosen perspective from sessionStorage
          const savedRole = sessionStorage.getItem("m3_perspective");
          if (savedRole) {
            setRole(savedRole);
  ...
  const handleSelectPerspective = (selectedRole) => {
    const found = getModule1User();
    if (!found) return;

    setAuthToken(found.token);
    const fullName = [m1User?.payload?.first_name, m1User?.payload?.last_name]
      .filter(Boolean).join(" ") || m1User?.payload?.email || found.payload.email;
    const user = {
      id: found.payload.uuid || found.payload.id,
      role: selectedRole,
      name: fullName,
    ...
    sessionStorage.setItem("m3_perspective", selectedRole);
    setScreen("home");
  };
  ...
  const onNavigate = (key, params = {}) => {
    if (key === "switch_role") {
      ...
      setScreenParams({});
      sessionStorage.removeItem("m3_perspective");
      // Keep the token — just clearing the perspective choice
      return;
    }
  ...
  // Token found but perspective not yet chosen
  if (!role) {
    return <PerspectiveSelection onSelectPerspective={handleSelectPerspective} m1User={m1User} />;
  }
