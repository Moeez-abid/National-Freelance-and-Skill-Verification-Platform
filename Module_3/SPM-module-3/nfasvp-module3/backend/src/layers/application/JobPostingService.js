...
async function createJob(clientId, jobData) {
  ...
  if (Array.isArray(required_skills) && required_skills.length > 0) {
    const stringTags = required_skills
      .map(s => typeof s === 'string' ? s : s.tag_id)
      .filter(s => typeof s === 'string' && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(s));
    
    let dbTags = [];
    if (stringTags.length > 0) {
      const { data } = await categoryRepo.upsertTags(stringTags);
      dbTags = data;
    }
    ...
    const skills = required_skills.map((s) => {
      if (typeof s === 'string') {
        ...
        if (isUuid) return { tag_id: s };
        const slug = s.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
        const found = dbTags.find(t => t.slug === slug);
        return found ? { tag_id: found.id } : null;
      }
      return { tag_id: s.tag_id, level: s.level };
    }).filter(Boolean);
